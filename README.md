...
```
proj_1/
├── main.c                  # Firmware entry point: driver init, sensor bring-up, main loop
├── ADuCRF101.h             # Analog Devices register definition header (CMSIS)
├── system_ADuCRF101.c      # System clock and core initialization
├── system_ADuCRF101.h      # System initialization interface
├── startup_ADuCRF101.s     # ARM Cortex-M3 startup assembly (vector table, reset handler)
└── proj_1.uvprojx          # Keil µVision 5 project file
```

> `Objects/` and `Listings/` are build output folders generated by Keil and are not tracked.
